...
<td><s:a name="orderIdLinks" id="idUrl348492393" target="_blank"><s:property value="deal.id"/></s:a></td>
...
<td><span class="ftx-03"><s:property value="deal.createTime"/></span></td>
<s:if test="deal.status == @sus.scrofa.entity.Deal@STATUS_CREATED">
<td><span class="ftx-03">待买家付款</span></td>
<td id="operate348492393" class="order-doi"><s:a action="ProcessPay" namespace="/deal"><s:param name="dealId" value="deal.id"/>去付款</s:a></td>
</s:if><s:elseif test="deal.status == @sus.scrofa.entity.Deal@STATUS_PAID">
<td><span class="ftx-03">待商家发货</span></td>
<td id="operate348492393" class="order-doi"><a href="#" >提醒商家发货</a></td>
</s:elseif><s:elseif test="deal.status == @sus.scrofa.entity.Deal@STATUS_DELIVERED">
<td><span class="ftx-03">待确认收货</span></td>
<td id="operate348492393" class="order-doi"><s:a action="ProcessReceive" namespace="/deal" ><s:param name="dealId" value="deal.id"/>确认收货</s:a></td>
</s:elseif><s:else>
<td><span class="ftx-03">已完成</span></td>
<s:if test="deal.status == @sus.scrofa.entity.Deal@STATUS_BUYER_COMMENTED">
<td id="operate348492393" class="order-doi"><s:a action="Detail" namespace="/show"><s:param name="id" value="showPic.id"/>查看晒图</s:a></td>
</s:if><s:else>
<td id="operate348492393" class="order-doi"><s:a action="Add" namespace="/show"><s:param name="dealId" value="deal.id"/>晒图</s:a></td>
</s:else>
</s:else>
</tr>
</s:iterator>
	...
	<div class="m clearfix">
		<div class="m clearfix">
			<div class="pagin fr">
			<s:a action="List" namespace="/deal"><s:param name="page" value="1"/>首页</s:a>
			<s:a action="List" namespace="/deal" cssClass="prev"><s:param name="page" value="page > 1 ? page - 1 : 1"/>上一页<b></b></s:a>
			<s:a action="List" namespace="/deal" cssClass="next"><s:param name="page" value="page < data['_total_page'] ? page + 1 : data['_total_page']"/>下一页<b></b></s:a>
			<s:a action="List" namespace="/deal"><s:param name="page" value="data['_total_page']"/>尾页</s:a>
			</div>
		</div>
	</div>
